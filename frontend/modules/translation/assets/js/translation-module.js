...
//		$(transLine).children('textarea').focus();
	...
	function addTransIcons(){
		if ($('.trble').length > $('.edit_translation').length){
			$.each($('.trble'), function(index, value){
				if ($(value).find('.edit_translation').length === 0){
	...
	function addGlobeIcons(){
		if (($('.trglobe').length > 0) && ($('.globe_translation').length === 0)){
			$.each($('.trglobe'), function(index, value){
				if ($(value).find('.globe_translation').length === 0){
