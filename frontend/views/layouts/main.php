...
<!--	<meta property="og:site_name" content="--><?php //echo Yii::t('settings', 'SAF') ?><!--" />-->
    <?php $this->head() ?>
	<!-- Google Tag Manager -->
...
<div class="admin_menu_fix">
    <div class="admin_menu_controls">
        <div class="site_name_admin">
            <a href="/site/admin/" target="_blank"><?= Yii::$app->name ?></a>
        </div>
        <div class="update_admin_btn">
			<?php
				if(!empty($this->context->actionParams['id'])){
					echo Html::tag('a', Html::encode(Yii::t('blog', 'EDIT_PAGE')), ['target' => '_blank','href' => '/'.$this->context->module->id.'/'.$this->context->id.'/update?id='.$this->context->actionParams['id']]);
				}
			?>
        </div>
    </div>
...
<div class="wrap">
    <div class="main_menu_s">
		<!-- <div class="mobile_with_container">
			<div class="phone_pre_container">
				<svg><use xlink:href="#phone_up"></use></svg>
				<a href="tel:<?= SinglecontactphoneWidget::widget(['telephony' => true,]) ?>">[single_contact_phone]</a>
			</div>
			<div class="order_pre_container">
				<svg><use xlink:href="#order_service"></use></svg>
				[button title="<?= Yii::t('blog', 'START_PROJECT'); ?>" formname=orderService extraclass=order_layout id=layoutStartProject]
			</div>
			<div class="lang_switch_container">
				<?php
					echo LanguageSwitcher::widget([
						'options' => ['class' => 'my_custom_lang_switch']
					]);
				?>
			</div>
		</div> -->
		...
		<div class="copyright">
			<div class="copyright__container">
				...
				<div class="copyright_col">
					<?php
					echo Yii::t('blog', 'ALL_RIGHTS_RESERVED') .'. SAF '. date('Y');
					?>
				</div>
