...
$this->title = Yii::t('blog', 'RESET_CREATE_PASSWORD_LOGIN');
$this->registerMetaTag(['property' => 'og:description', 'content' => Yii::t('blog', 'RESET_CREATE_PASSWORD_LOGIN')]);
$this->registerMetaTag(['name' => 'description', 'content' => Yii::t('blog', 'RESET_CREATE_PASSWORD_LOGIN')]);
?>
<div class="login__container">
	<h1 class="login__title"><?= Yii::t('blog','RESET_CREATE_PASSWORD_LOGIN') ?></h1>

	<?php $form = ActiveForm::begin(['id' => 'reset-password-form']); ?>

	<?php
	if(!empty($email)){
		echo Html::hiddenInput('email', $email->email);
	}
	?>

	<div class="login__box">
		<svg><use xlink:href="#svg_login_user"></use></svg>
		<?= $form->field($model, 'password')->passwordInput(['class'=>'login__input', 'autofocus' => true,'placeholder' => Yii::t('blog','CREATE_PASSWORD')])->label(false) ?>
	</div>
